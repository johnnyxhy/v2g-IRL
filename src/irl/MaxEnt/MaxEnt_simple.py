...
class MaxEntConfig:
    """Configuration for MaxEnt IRL with PPO inner loop (simple env)."""
    ...
    policy_ent_coef: float = 0.01     # Entropy bonus (fixed)
...
class MaxEntIRLTrainer_Simple(MaxEntIRLTrainerBase):
    """
    MaxEnt IRL with PPO as the inner-loop policy optimizer for the
    simple V2GEnv_simple environment (discrete actions: charge/discharge/idle).

    The reward is linear in hand-crafted features (no price):
        R(s, a) = w · φ(s, a)

    The IRL gradient is:
        ∇w = (1/N) Σ_i [ φ_expert^(i) − Σ_j softmax(R(τ_j)) · φ(τ_j) ]

    Feature set (5 features, no price):
    ...
    def __init__(self,
        ...
        # Per-feature scale from expert data for gradient normalisation
        ...
        print(f"MaxEnt IRL (Simple PPO) Trainer: "
    ...
    def _plot_results(self):
        ...
        plt.title('MaxEnt IRL (Simple) - Feature L2 Loss')
        ...
        plt.title('MaxEnt IRL (Simple) - SoC MAE')
        ...
        plt.title('MaxEnt IRL (Simple) - DTW Distance')
        ...
        plt.title('MaxEnt IRL (Simple) - Expert Log-Likelihood')
        ...
        feature_names = [
            FEATURE_NAMES[i] if i < len(FEATURE_NAMES) else f'feature_{i+1}'
            for i in range(n_weights)
        ]
        self._plot_weights_evolution(feature_names)
