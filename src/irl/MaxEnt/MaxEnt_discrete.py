...
class FlattenNormalizeObsWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        super().__init__(env)
        obs_dim = len(DISCRETE_OBS_SCALES)
        self.observation_space = gym.spaces.Box(
            low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32
        )
        self._scales = np.array(DISCRETE_OBS_SCALES, dtype=np.float32)
...
class MaxEntConfig:
    """Configuration for MaxEnt IRL with PPO inner loop (discrete env)."""
    ...
    policy_clip_range: float = 0.2   # PPO clip range
...
class MaxEntIRLTrainer_Discrete_PPO:
    """
    ...
    PPO is used instead of SAC because the action space is Discrete(3).
    SB3's PPO supports MultiInputPolicy natively (Dict obs + discrete actions),
    so no observation flattening wrapper is required.

    Key differences from the SAC continuous variant:
    - PPO is on-policy: no replay buffer or entropy coefficient to reset.
    - reset_num_timesteps=False lets PPO accumulate experience across IRL epochs.
    - One action = one timestep (fixed dt), so no variable-dt discounting needed.
    """
    ...
    def __init__(self,
        ...
        self.train_set, self.val_set = expert_trajectories.split_dataset(
        ...
        # Each feature's gradient is divided by its std so that no single
        # high-magnitude feature (e.g. battery_exceeded_target ≈ 8×) can
        # dominate the IRL weight update.
        ...
        print(f"MaxEnt IRL (Discrete PPO) Trainer: "
              f"{len(self.train_set)} train, {len(self.val_set)} val trajectories.")
    ...
    def train(self):
        ...
        # Write about.md description file (only if it doesn't already exist)
        ...
        train_wrapped = FlattenNormalizeObsWrapper(train_base_env)
        ...
        rollout_wrapped = FlattenNormalizeObsWrapper(rollout_base_env)
        rollout_env = DummyVecEnv([lambda: rollout_wrapped])

        # PPO policy — SBX PPO only supports MlpPolicy; flat obs provided by wrapper
        ...
        for epoch in range(cfg.n_epochs):
            ...
            # Per-feature normalisation: divide by expert feature std so that
            # high-magnitude features don't dominate the weight update.
            grad = grad / self.feat_scale

            # Gradient clipping
            grad_norm = np.linalg.norm(grad)
            if cfg.grad_clip_norm is not None and grad_norm > cfg.grad_clip_norm:
                grad = grad * (cfg.grad_clip_norm / grad_norm)

            # Linear LR decay
            if cfg.n_epochs > 1:
                current_lr = cfg.reward_lr + (cfg.reward_lr_end - cfg.reward_lr) * (
                    epoch / (cfg.n_epochs - 1)
                )
            else:
                current_lr = cfg.reward_lr

            # Gradient ascent on reward weights (with L2 regularisation)
            self.reward_weights = self.reward_weights + current_lr * (grad - cfg.reg_lambda * self.reward_weights)
    ...
    def _plot_results(self):
        ...
        plt.title('MaxEnt IRL (Discrete PPO) — L2 Loss')
        ...
        plt.title('MaxEnt IRL (Discrete PPO) — DTW Distance')
        ...
        for i in range(n_weights):
            ...
            axes_flat[i].set_title(f'Weight {i+1}')
